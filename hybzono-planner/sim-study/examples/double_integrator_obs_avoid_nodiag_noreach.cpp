...
int main()
{
    ...
    file.open("../examples/data/solver_nodiag_noreach.txt");
    file_traj.open("../examples/data/solver_nodiag_noreach_traj.txt");
    ...
    for (const int & n_horizon : n_horizon_arr)
    {
        ...
        for (auto const & map : map_vec)
        {
            ...
            for (int i=0; i < n_trials; i++)
            {
                ...
                // distance limit
                mpc_ours_hz->set_distance_limit(QP::inf); // NO REACHABILITY CONSTRAINTS
