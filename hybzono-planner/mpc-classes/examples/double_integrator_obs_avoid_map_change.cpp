...
// open map
void get_open_map(std::pair<double,double> x_range, std::pair<double,double> y_range,
    HybZono &X, 
    std::map<int, Eigen::MatrixXd> &A_Hrep, std::map<int, Eigen::VectorXd> &b_Hrep)
{
    // free space grid
...
int main()
{
    ...
    // terminal state constraints, reference dependent
    Eigen::Matrix<double, 4, 4> Gx_term = Eigen::Matrix<double, 4, 4>::Zero();
    Gx_term.diagonal() << 30, 0, 30, 0;
    Eigen::Vector<double, 4> cx_term = Eigen::Vector<double, 4>::Zero();
    Eigen::Matrix<double, 0, 4> Ax_term = Eigen::Matrix<double, 0, 4>::Zero();
    Eigen::Vector<double, 0> bx_term = Eigen::Vector<double, 0>::Zero();
    ConZono Zc_x_term (Gx_term.sparseView(), cx_term, Ax_term.sparseView(), bx_term);
    Eigen::Matrix<double, 4, 4> Pp_x_term = Eigen::Matrix<double, 4, 4>::Identity();

    Eigen::Matrix<double, 4, 4> Qx_term_ineq_cost = 1e6*Eigen::Matrix<double, 4, 4>::Identity();
    Eigen::Vector<double, 4> sigma_max_x_term = 1e4*Eigen::Vector<double, 4>::Ones();
    ...
    // create map
    HybZono X_hz_1, X_hz_2;
    std::map<int, Eigen::MatrixXd> A_Hrep_1, A_Hrep_2;
    std::map<int, Eigen::VectorXd> b_Hrep_1, b_Hrep_2;
    std::pair<double,double> x_range = std::make_pair(-15, 15);
    std::pair<double,double> y_range = std::make_pair(-10, 10);
    get_open_map(x_range, y_range, X_hz_1, A_Hrep_1, b_Hrep_1);
    get_map_zonotope_tiling(x_range, y_range, X_hz_2, A_Hrep_2, b_Hrep_2);
    ...
    mpc.set_terminal_state_inequality_constraints(Zc_x_term, Pp_x_term.sparseView(), x_min_hard, x_max_hard); 
    mpc.set_terminal_state_inequality_constraints_slack_vars_cost(Qx_term_ineq_cost.sparseView(), sigma_max_x_term);

    mpc.setup_hybzono_Hrep_constraints(X_hz_1, A_Hrep_1, b_Hrep_1, Pc.sparseView(), Q_hz.sparseView());
    ...
    mpc_settings.ref_dep_term_constraint = true;
    ...
    // time execution
    double tot_time = 0, max_time = 0, map_update_time = 0;
    ...
    for (int i = 0; i < n_cycles; i++)
    {
        ...
        if (i == 5)
        {
            mpc.update_hybzono_Hrep_constraints(X_hz_2, A_Hrep_2, b_Hrep_2);
        }
        auto map_duration = std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::high_resolution_clock::now() - start_map);
        if (i == 5)
        {
            map_update_time = map_duration.count()/1e6;
        }
    ...
    std::cout << "Map update time: " << map_update_time << " sec" << std::endl;
